...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        # q, k, v: [batch_size, session_length(lq), dim]
        ...
        # Pass through the pre-attention projection: batch_size x lq x (n_head * dv)
...
class TransAm(nn.Module):
    def __init__(self, dim, num_layers=1, dropout=0.001):
        ...
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=dim, nhead=2, dropout=dropout, batch_first=True)
        self.Encoder = nn.TransformerEncoder(self.encoder_layer, num_layers=num_layers)

        self.decoder_layer = nn.TransformerDecoderLayer(d_model=dim, nhead=2, dropout=dropout,
                                                        dim_feedforward=4 * dim, batch_first=True)
        self.Decoder = torch.nn.TransformerDecoder(self.decoder_layer, num_layers=num_layers)
